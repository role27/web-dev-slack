...
public class B_Aggregate {
	...
	/*
	 * 기본집계
	 * - 최종 처리 기능으로 요소들을 개수, 합계, 평균값, 최대갑스, 최소값 등과 같이 하나의 값으로 산출하는 역할
	 * - count : 요소의 개수 리턴
	 * - sum : 요소들의 합계 리턴
	 * - average : 요소들의 평균 리턴
	 * - max : 최대 요소 리턴
	...
	public void method1() {
		 ...
		 * - 단순히 값만 저장하는 것이 아니라, 값의 존재 여부 확인하고 값이 존재하지 않는 경우 디폴트 값을 설정할 수 있는 객체
		 * 
		 * */
		...
		//ifPresent : 값이 저장되어 있을 경우만 처리 (if + isPresent)
	...
	/*
	 * 커스텀 집계
	 * - 스트림에서 기본 집계 메서드를 제공하지만 다양한 집계 결과물을 안들 수 있도록 reduce 메서드 제공
	 * 
	 * */
