/*
 ...
 - DBMS : Data Base Management System의 약자, 데이터베이스 관리 시스템
...
SELECT는 데이터 조회를 의미하여 일부에서는 DQL(Date Quety Language)로 분류하기도 함
...
SELECT 
NAME 이름, AGE 나이, AGE + 10 AS "10년 후" 
FROM USER_INFO;
...
- ESCAPE ONTION : 나만의 와일드카드 만들어서 사용!
...
-- USER_INFO 에서 MBTI가 INFP 또는 INTJ인 사용자 조회
SELECT *
FROM USER_INFO
--WHERE MBTI='INFP' OR MBTI='INFJ';
WHERE MBTI IN('INFP','INTJ');

-- EMPLOYEE에서 부서코드(DEPT_CODE)가 D5,D6,D8인 사람들 조회
...
/*
ORDER BY
- SELCET문 가장 마지막 줄에 작성뿐만 아니라 실행순서 또한 마지막
...
FROM EMPLOYEE
--ORDER BY BONUS; NULLS FIRST; --오름차순인 경우 NULL이 맨뒤
ORDER BY BONUS DESC NULLS LAST; -- 내림차순인 경우 NULL이 맨앞에
...
-- EMPLOYEE에서 부서별 평균 급여가 300만원 이상인 직원의 평균 급여를 조회
-- 급여 SALARY / 부서코드 DEPT_CODE
-- DEPT_CODE가 NULL이 아닌경우
