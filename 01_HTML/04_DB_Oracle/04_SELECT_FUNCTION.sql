...
-CUBE(컬럼1, 컬럼2) : 컬럼1을 가지고 중간집계를 내고, 컬럼2를 가지고 중간집계를 또 내는 함수
...
SELECT JOB_CODE, SUM(SALARY)
FROM EMPLOYEE
GROUP BY CUBE(JOB_CODE); -- 컬럼 하나일 때는 ROLLUP과 차이 없음


-- 부서코드와 직급코드가 같은 사원들의 급여합계
SELECT JOB_CODE, DEPT_CODE, SUM(SALARY)
FROM EMPLOYEE
GROUP BY JOB_CODE, DEPT_CODE;

--ROLLUP : DEPT_CODE 기준으로 합계
...
UNION(합집합) - 중복된 값은 제거 / UNION ALL(합집합)  - 중복된 값 포함
: 두 쿼리문을 수행한 결과값을 하나로 합쳐서 추출
...
-- 위 쿼리문 대신 WHERE절에서 OR연산자 사용해서도 처리가능
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
...
--3. INTERSECT
-- 부서코드가 'D5'이면서 급여가 300만원 초과인 사원들 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE='D5'
INTERSECT
...
-- 4. MINUS
-- 부서코드가 D5이면서 급여가 300만원 초과인 사원들 제외
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE='D5'
MINUS
